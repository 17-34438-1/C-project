...
namespace BankMain.Data_Acccess_Layer
{
    public class DataAccess
    {
        ...
        public DataAccess()
        {
            con = new SqlConnection(@"Data Source=DESKTOP-RQ4CJI3;Initial Catalog=Bank;Integrated Security=True");
        ...
        {
            int i = 0;
            string query = "INSERT INTO Users([Full Name],Password,[Phone Number],DOB,Address,[Account Type],[National Id],Email,Gender,Signeture,Balance,Pic,[DipositeDate],[WithdrawDate],Loan) VALUES (' " + u.UserName + " ',' " + u.Password + " ', ' " + u.PhoneNumber + " ', ' " + u.DOB + " ',' " + u.Address + " ',' " + u.AccountType + " ',' " + u.NationalId + " ',' " + u.Email + " ', ' " + u.Gender + " ', @Sig, ' " + u.Balance + " ',@Pict, ' " + u.DipositeDate + " ', ' " + u.WithdrawDate + " ', ' " + u.Loan + " ')";
            SqlCommand cmd = new SqlCommand(query, con);
        ...
        {
            int i = 0;
            string query = string.Format("UPDATE users SET Diposited='{0}', Balance='{1}',[DipositeDate]='{2}' WHERE [Account Number]={3}", u.Dipsited, u.Balance,u.DipositeDate, u.UserId);
            SqlCommand cmd = new SqlCommand(query, con);
            i=cmd.ExecuteNonQuery();
            //con.Close();
            return i;
        ...
        {
            int i = 0;
            string query = string.Format("UPDATE users SET Withdrawn='{0}', Balance='{1}',[WithdrawDate]='{2}' WHERE [Account Number]={3}", u.Withdrawn, u.Balance,u.WithdrawDate, u.UserId);
            SqlCommand cmd = new SqlCommand(query, con);
            i = cmd.ExecuteNonQuery();
            //con.Close();
            return i;
